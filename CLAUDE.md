...
- De gesimuleerde bronuitval (feature-flag "Berichtenbox unhappy flow", cookie) hoort bij de dataset-bron. Een bron die niet antwoordt levert geen berichten, dus laat `laad()` ze weg in plaats van dat de render-laag ze wegfiltert — zo is een nagebootste storing hetzelfde als een echte. Drie scenario's, één per weergave gekozen: `een` (één magazijn stil), `geen` (geen enkel) en `later` (alles laadt, daarna valt er één weg — dat laatste is `start`-gedrag en meldt zich als gewone bronwijziging). Wat er wegviel staat in het veld `uitval` van het laad-antwoord en van elke bronwijziging; `bron.uitval()` is er voor pagina's die niet laden, zoals de detailpagina. De sessionStorage-sleutel `berichtenbox-bron-uitval` zit in de bron. `bron.herstelBronnen()` hoort onder de knop "Opnieuw proberen", `bron.vergeetUitval()` onder het uitzetten van de vlag. De drie meldingsblokken in de template blijven waar ze zijn; de render-laag leest alleen nog de stand.
