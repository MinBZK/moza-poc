...
- Berichtenbox, Federatief Berichtenstelsel: `assets/javascript/berichtenbox-keten.js` haalt voor een aangesloten persona de berichten uit de keten in plaats van uit de gegenereerde dataset. Het script draait vóór `berichtenbox.js` (volgorde in `_includes/base.njk`), herstelt een eerdere ronde synchroon uit `localStorage` (key `berichtenbox-keten`, zodat archief, prullenbak en detailpagina's hun bericht vinden — bewust dezelfde opslag als `state`, dat naar diezelfde berichtIds verwijst) en draait op de inbox een nieuwe ronde asynchroon. `berichtenbox.js` rendert eerst de dataset en neemt de keten-berichten daarna over via `window.Berichtenbox.ketenOvername`; het blijft dus een synchrone IIFE, zonder `await` op de kritieke render. Zolang de keten de lijst bepaalt (`ketenActief`) staat de demo-simulatie uit: geen voortgangsanimatie, geen binnendruppelende berichten, geen gesimuleerde bronuitval — verzonnen berichten tussen echte zijn voor de bezoeker niet te onderscheiden. Endpoints (same-origin, via nginx): `/api/v1/berichten/_ophalen` (SSE-ophaalronde), `/api/v1/berichten` (lijst), `/api/demo/personas` (welke persona's de keten kent). Runtime-vars: `BACKEND_KETEN` (valt terug op `BACKEND_ORIGIN`), `BACKEND_DEMO` (valt terug op `BACKEND_KETEN`) en, bij een backend achter een gedeelde ingress, `BACKEND_KETEN_HOST` / `BACKEND_DEMO_HOST` voor de Host-header (leeg = eigen host doorgeven). De ontvanger gaat mee als header `X-Ontvanger` (`KVK:<nummer>`, uit `window.Personas.actief()`). Let op: dit is **geen authenticatie** — de header is in de browser aan te passen, dus het stelsel moet zijn eigen allowlist hanteren; zelfde afweging als bij `X-Test-User`. Testpersona's: `proeftuin-een` (90000011), `proeftuin-twee` (90000012), `proeftuin-drie` (90000013) en `proeftuin-veel` (90000001). Faalt een ronde voor een persona die aantoonbaar aangesloten is, dan verdwijnt de dataset-lijst en blijft alleen de melding over — niet stil terugvallen
