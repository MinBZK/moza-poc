...
- **Twee lagen**; de tokens zijn opgedeeld in _opties_ (de beschikbare waarden uit de Rijkshuisstijl) en _toepassingen_ (semantische verwijzingen die bepalen hoe de opties worden ingezet). In de code worden altijd toepassingsvariabelen gebruikt, nooit opties rechtstreeks.
...
Dit principe gaat over de **interface die de gebruiker ziet**: de pagina's, componenten en interacties in de browser. Daar is elke extra laag iets waar de gebruiker last van kan krijgen; in bijvoorbeeld laadtijd, toegankelijkheid, wat er stukgaat als JavaScript niet laadt. Dit geldt niet voor de laag eromheen zoals build-scripts, datatransformaties, koppelingen met backends en testopstellingen.

- **HTML en CSS waar het kan, JavaScript waar het moet**; in de interface lossen standaard browsergedrag en semantische HTML veel interactieproblemen op zonder dat hier aanvullende code voor nodig is. JavaScript wordt alleen toegevoegd wanneer de gewenste functionaliteit niet met HTML en CSS te realiseren is.
- **Platform boven framework**; het ontwerp leunt op wat de browser al biedt: native formuliervalidatie, `<details>`/`<summary>` voor in- en uitklapbare secties, CSS voor responsiviteit. Frameworks en bibliotheken worden in de interface alleen ingezet als het platform aantoonbaar niet volstaat.
...
De inzet van AI-assistentie past bij het principe van _de eenvoudigst mogelijke oplossing_: het versnelt het werk zonder extra tooling, frameworks of afhankelijkheden te introduceren. De gegenereerde code en content worden altijd beoordeeld en waar nodig aangepast door de ontwerper.
...
- **Togglebaar via het paneel**; rechtsonder in het prototype bevindt zich een "Flags"-knop die een paneel opent met alle beschikbare feature flags, gegroepeerd in _Pagina's_ (navigatie-items en secties) en _Functionaliteit_ (interactiemogelijkheden zoals delen en relevantie aangeven).
