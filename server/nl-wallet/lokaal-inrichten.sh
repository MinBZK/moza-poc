...
# lokaal-inrichten.sh
#
# Richt de lokale NL Wallet-ontwikkelomgeving (github.com/MinBZK/nl-wallet) in
# als verifier voor MOZa: een WRPAC-certificaat voor "MijnOverheid Zakelijk",
...
# niet ook in de pre-prod-wallet staan. Daarna starten de betrokken services opnieuw.
#
# Vereist: een NL Wallet-checkout waarin scripts/setup-devenv.sh al gedraaid heeft.
# Opnieuw draaien kan altijd; setup-devenv.sh overschrijft de configuratie, dus
...
NL_WALLET_DIR="${NL_WALLET_DIR:-$HOME/Documents/GitHub/NL wallet}"
MOZA_ORIGIN="${MOZA_ORIGIN:-http://localhost:8095}"
...
CRL_URL="http://localhost:3021/wrpac.crl.der"
...
NL_WALLET_APK="${NL_WALLET_DIR}/wallet_app/build/app/outputs/flutter-apk/app-debug.apk" python3 - "${IS_DIR}/issuance_server.toml" "${DEMO_ISSUER_JSON}" "${ATTESTATIES}/bevoegdheden.json" "${KVK_PREFIX}" "${SCRIPT_DIR}/lokaal.json" <<'PY3'
...
# Links voor "bevoegdheid toevoegen" per onderneming op de MOZa-pagina (lokaal, niet in git).
def link(u, sessie):
...
apk = os.environ.get("NL_WALLET_APK", "")
...
# Herstart zonder scripts/start-devenv.sh: dat draait bij elke start `migrations fresh` en gooit
# target/status-lists weg. Dan verdwijnen de statuslijsten van kaarten die al in een wallet staan (PID,
# bevoegdheden), en weigert de verification_server die kaarten bij het inloggen ("not all revocation
# statuses are valid"). Deze herstart gebruikt dezelfde startopdracht, maar laat database en lijsten staan.
herstart() {
	local package="$1" map="$2" log="$3" features="$4"
	pkill -f "${package}\$" || true
	(
		cd "${NL_WALLET_DIR}/wallet_core/${map}"
		RUST_LOG=debug nohup cargo run --quiet --package "${package}" ${features:+--features "${features}"} --bin "${package}" >"${TARGET_DIR}/${log}.log" 2>&1 &
	)
}

echo "gba_hc_converter, pid_issuer, issuance_server, demo_issuer en verification_server opnieuw starten (database en statuslijsten blijven staan)"
herstart gba_hc_converter gba_hc_converter gba_hc_converter ""
herstart pid_issuer wallet_server/pid_issuer pid_issuer "allow_insecure_url,test_internal_ui"
herstart issuance_server wallet_server/issuance_server demo_issuer_issuance_server "allow_insecure_url,test_internal_ui"
herstart demo_issuer demo/demo_issuer demo_issuer "allow_insecure_url"
herstart verification_server wallet_server/verification_server demo_rp_verification_server "allow_insecure_url"

echo "Klaar. Wacht tot pid_issuer (3003), issuance_server (3008), demo_issuer (3006, 3007) en verification_server (3011, 3012) weer luisteren."
