...
De CSS maakt gebruik van logical properties (`inline-size`, `margin-block-start`, `padding-inline`) in plaats van physical properties (`width`, `margin-top`, `padding-left`). Dit maakt het onder meer toekomstbestendig voor meertalige ondersteuning en verschillende schrijfrichtingen.
...
`<body>` is een flex column met `min-block-size: 100dvh`, en `<main>` heeft `flex: 1`. Daardoor staat `<footer>` altijd onderaan het venster bij korte pagina’s, en schuift die gewoon mee bij langere. `100vh` staat als fallback voor browsers zonder `dvh`-ondersteuning.
...
Het alternatief **`> * + *` met margin** is een geldige techniek voor content-flow zonder flex-context, maar wordt op dit moment niet gebruikt in deze codebase. Mocht het ooit weer nodig zijn: nooit combineren met `gap` op dezelfde container.
...
- **Interactie op inhoud**: het patroon waarmee gebruikers door middel van acties als Bewaar, Relevant, Niet relevant voor mij en Deel een profiel opbouwen dat de basis vormt voor gepersonaliseerde content (aanbevelingssysteem).
...
- **Onthouden in de browser**: feature flags en de actieve persona worden opgeslagen in de browser met [_localStorage_](https://developer.mozilla.org/en-US/docs/Web/API/Web_Storage_API/Using_the_Web_Storage_API). Standaard staan alle features aan; pas als een feature expliciet wordt uitgezet, wordt deze verborgen.
...
- **localStorage wissen**: het paneel bevat een knop om alle opgeslagen staten (feature flags, persona-keuze, bewaarde items, verborgen topics, gesloten notificaties) in één keer te resetten.
