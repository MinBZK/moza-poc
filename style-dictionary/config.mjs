...
async function run() {
	...
	const tsTransforms = StyleDictionary.hooks.transformGroups["tokens-studio"].filter((t) => t !== "ts/size/px");
		...
		platforms: {
			css: {
				...
				files: setNames.map((setName) => ({
					destination: `./style/tokens/_${setName}.css`,
	...
	try {
		execSync("npx stylelint --fix ./style/tokens/_rijkshuisstijl.css ./style/tokens/_toepassing.css", { stdio: "inherit" });
	} catch (e) {
